...
var topoJSONRoutes = new Tilesplash("postgres://" + credentials.user + ":" + credentials.passwd + "@ncwageoservice.ccts9i164ms9.us-east-1.rds.amazonaws.com/ncwadb");
...
topoJSONRoutes.layer('auall', corsMiddleware, function(tile, render){
...
topoJSONRoutes.layer('countiesall', corsMiddleware, function(tile, render){
  this.cache(1000 * 60 * 60 * 24); //cache for one day
  render('SELECT ST_AsGeoJSON(geom) as the_geom_geojson FROM counties_ca;');
});

topoJSONRoutes.layer('gwlall', corsMiddleware, function(tile, render){
  this.cache(1000 * 60 * 60 * 24); //cache for one day
...
topoJSONRoutes.layer('gwlfiltered', corsMiddleware, function(tile, render){
	this.cache(1000 * 60 * 60 * 24); //cache for one day
